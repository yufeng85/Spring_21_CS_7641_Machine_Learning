...
#plt.ylim(0,40)
plt.grid(True)
plt.savefig('DT_sample_A_Learning_curves_for_a_decision_tree_Default_setting.png',dpi=600)
...
plt.grid(True)
plt.savefig('DT_sample_A_Post_pruning_Total_Impurity_vs_effective_alpha_for_training_set.png',dpi=600)
...
ax[1].grid(True)
fig.tight_layout()
plt.savefig('DT_sample_A_Post_pruning_nodes_and_depth_vs_alpha.png',dpi=600)

#==============================================================================
#Gid search
#==============================================================================
parameters = {'criterion':['gini', 'entropy'], 'max_depth':[4, 6, 8], 'ccp_alpha':[0, 0.01, 0.02, 0.03, 0.04]}
dt_clf = DecisionTreeClassifier(random_state=0)
...
max_depth_range = np.linspace(2, 12, 11)
ccp_alpha_range = np.linspace(0, 0.04, 5)
...
plt.imshow(scores, interpolation='nearest', cmap=plt.cm.hot,
           norm=MidpointNormalize(vmin=0.72, midpoint=0.74))
...
plt.grid(False)
plt.savefig('DT_sample_A_Grid_search.png',dpi=600)
plt.show()
...
plt.legend(loc="best")
plt.grid(True)
plt.savefig('DT_sample_A_Validation_Curve_Max_depth.png',dpi=600)
plt.show()
...
#==============================================================================
dt_clf = DecisionTreeClassifier(random_state=0)
param_range = [0, 0.01, 0.02, 0.03, 0.04]
train_scores, test_scores = validation_curve(
...
plt.legend(loc="best")
plt.grid(True)
plt.savefig('DT_sample_A_Validation_Curve_ccp_alpha.png',dpi=600)
plt.show()
...
train_sizes, train_scores, validation_scores = learning_curve(
estimator = DecisionTreeClassifier(random_state=0, criterion='gini', max_depth=5, ccp_alpha=0.02),
X = x_train,
y = y_train, train_sizes = train_sizes, cv = 5,
scoring = 'accuracy',
shuffle = True,
...
#plt.axis('on')
plt.grid(True)
plt.savefig('DT_sample_A_Learning_curves_for_after_hyper_parameter_tuning.png',dpi=600)
...
print('\n', '-' * 50)
print('After hyperparameter tunning, criterion=gini, max_depth=5, ccp_alpha=0.02')
